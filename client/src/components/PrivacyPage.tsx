...
function LanguageToggle() {
  ...
  return (
    <div className="inline-flex rounded-full border border-gray-200 bg-gray-50/80 p-1 text-sm shadow-sm" role="group" aria-label="Language">
      <button
        type="button"
    ...
    <div className="min-h-screen bg-white">
      <header className="w-full px-6 md:px-12 py-5 border-b border-gray-100">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 md:items-center">
          ...
          <nav className="flex flex-wrap items-center justify-center gap-6 text-sm font-medium text-gray-600 md:gap-8">
            <Link to="/" className="transition-colors hover:text-[#2E7D5B]">Home</Link>
            <Link to="/privacy" className="text-[#2E7D5B]">Privacy</Link>
            <a href="/#about-us" className="transition-colors hover:text-[#2E7D5B]">About Us</a>
            <a href="/#contact-us" className="transition-colors hover:text-[#2E7D5B]">Contact Us</a>
          </nav>
      ...
      <section className="bg-[#ecf7f1] py-16 md:py-24">
        <div className="mx-auto max-w-6xl px-6 md:px-12">
          <h1 className="text-3xl md:text-4xl font-bold text-[#18382b]">
            {isSl ? "Vaša zasebnost" : "Your privacy"}
          </h1>
          <p className="mt-2 text-lg text-[#2b5b46] font-medium">
            {isSl ? "Zaščita, ki ji lahko zaupate." : "Protection you can trust."}
          </p>
          <p className="mt-5 max-w-4xl text-gray-700 leading-relaxed">
            {isSl
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {[isSl ? "Strežniki v Sloveniji" : "Servers in Slovenia", "GDPR", isSl ? "Šifriranje od konca do konca" : "End-to-end encryption"].map((tag) => (
              <span key={tag} className="rounded-full border border-[#b8dbc9] bg-white px-4 py-2 text-sm font-medium text-[#2b5b46]">
                {tag}
              </span>
            ))}
          </div>
      ...
      <section className="bg-white py-14 md:py-20">
        <div className="mx-auto max-w-6xl space-y-12 px-6 md:px-12">
          {[
            ...
            <article
              key={item.num}
              className={`grid items-center gap-8 rounded-3xl border border-[#dceee4] p-6 md:grid-cols-2 md:p-10 ${idx % 2 === 0 ? "bg-[#f4fbf7]" : "bg-white"}`}
            >
              <div className={idx % 2 === 1 ? "md:order-2" : ""}>
                <p className="text-sm font-semibold tracking-wider text-[#2E7D5B]">{item.num} — {item.title}</p>
                <h3 className="mt-2 text-2xl font-bold text-gray-900">{item.subtitle}</h3>
                <p className="mt-4 text-gray-700 leading-relaxed">{item.p1}</p>
                <p className="mt-3 text-gray-700 leading-relaxed">{item.p2}</p>
              </div>
              <div className={`flex justify-center ${idx % 2 === 1 ? "md:order-1" : ""}`}>
                <img src={item.img} alt="" width={512} height={512} loading="lazy" className="h-48 w-48 object-contain md:h-56 md:w-56" />
              </div>
            </article>
          ))}
        </div>
